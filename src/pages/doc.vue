  ...
  <section class="page-all" v-loading = "loadingList">
    <section class="all-header">
      ...
      <section class="header-right">
        <p class="header-right-search"><input type="text" name="search" placeholder="搜索您的文件" v-model.trim="searchName" @keyup.enter="search" /><i class="icon icon-search" @click="search" ></i>
        <i class="icon el-icon-close" @click="searchName=''" v-if="searchName"></i></p>
    ...
    methods:{
      ...
      search(){
        if (this.searchName.trim().length==0) {
          ...
          this.$router.push({
            path: '/search',
            query: {id: '-1', querystring: this.searchName,fileType:'3'}
          })
        }
      },
      // callGoogle() {
      ...
      getTreeData(data) {
        if(this.offlineDownFlag){
          //离线下载获取保存路径
          this.offlineSavePath = data;
          return ;
        }
        if(this.singleData){
          //判断是否是通过更多图标操作
          this.$refs.bList.moveOrCopy(data,this.singleData,this.singleAction);
          this.singleData = null;

        }else{
          this.$refs.bList.moveOrCopy(data);
        }
        // console.log( this.$refs.bList );
