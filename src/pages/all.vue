    ...
    <section class="all-header">
      ...
      <section class="header-right">
        <p class="header-right-search">
          <input type="text" name="search" placeholder="搜索您的文件" v-model.trim="searchName" @keyup.enter="search"/>
        ...
        <p class="header-right-layout"><i :class="['icon iconfont',this.styleFlag ? 'icon-tb':'icon-lb']"
                                          @click="changeStyle"></i></p>
      </section>
    </section>
    <!-- <Operation /> -->
    <!-- <Dialog :showDialog="showDialog" :files="files" @close="showDialog = !showDialog" @uploaded="uploaded"
            :catId="queryData.id"/> -->
    <List
      :params="queryData"
      :url="url"
    ...
    methods: {
      search() {
        ...
        if (this.searchName.trim().length==0) {
          ...
          this.$router.push({
            path: '/search',
            query: {id: '-1', querystring: this.searchName,fileType:'1'}
          })
        }
      },
      // callGoogle() {
