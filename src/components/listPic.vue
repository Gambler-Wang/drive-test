            ...
            <section class="filename-box">
              ...
              <section>
                ...
                <p v-else class="text-overflow">
                  <a href="javascript:;" class="text-overflow"  v-maxLength = "item.name"></a>
      ...
      <dl class="card-content" v-else>
        ...
        <dd v-for="(item,key) of dataList" :key="key" @click="checkTag(item,key)"  :class="{'checked':item.checked}">
          ...
          <section class="filename-box">
            ...
            <p v-else class="text-overflow">
              <a href="javascript:;"  v-maxLength = "item.name"></a>
            </p>
          </section>
          <p class="p-checkbox">
  ...
  export default {
    ...
    methods: {
      ...
      submit(item) {
        ...
        const a =
          this.$refs.rename[0].value +
          item.name.slice(item.name.lastIndexOf("."), item.name.length);
        const b = this.$refs.rename[0].value;
        const params = {
          name: item.fileType != 1 ? a : b,
          id: item.id,
          parentId: this.$route.query.id ? this.$route.query.id : -1,
          parentName: this.$route.query.filename ? this.$route.query.filename : ""
        };
        this.$http({
        ...
        this.$http({
          ...
          .then(res => {
            ...
            if (res.data.code === 100001) {
              console.log(res);
              // 判断单文件多文件
              if (res.data.data.files.dataList.length > 1) {
                this.tt = Object.assign({}, { id: 2 });
              } else if (
                res.data.data.files.dataList.length == 1 &&
                res.data.data.files.dataList[0].fileType == 1
              ) {
                this.tt = Object.assign({}, { id: 2 });
              } else {
                this.tt = Object.assign({}, { id: 1 });
                this.type = res.data.data.files.dataList[0].fileType;
                localStorage.setItem('fileInfo',JSON.stringify(res.data.data.shareInfo));
              }
              let params = {
                urlCode: item.urlCode.slice(
                  item.urlCode.lastIndexOf("shareinit") + 10,
                  item.urlCode.length
                ),
                id: item.id
                    ...
                    if (res.data.data.isOwner == 1) {
                      ...
                      if (this.tt.id == 2) {
                        ...
                        this.$router.push({
                          path: "/shareDetail",
                          query: {
                            ids: this.dataList[index].fileIds[0],
                            id: params.urlCode,
                            type: this.type,
                            one: item.id
                          }
...
<style lang="scss" scoped>
  .component-list {
    ...
    & .p-checkbox {
      ...
      & > input[type="checkbox"]:checked + i {
        background: url(~@/assets/images/tips_password_icon_right.png) center
        center no-repeat;
  ...
  .component-list .list-content > dt > section:hover, .component-list .list-content > dt > span:hover {
    background-color: #e9f0f5;
  }
</style>
